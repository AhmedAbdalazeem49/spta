          ...
          {isLoading ? (
            ...
            >
              ...
              <h3 className="text-2xl font-bold mb-2">
                {t("لا توجد باقات متاحة حالياً", "No plans available yet")}
              </h3>
              <p className="text-muted-foreground">
                {t("يرجى المحاولة لاحقاً", "Please check back later")}
              </p>
            </motion.div>
          ) : (
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
                    ...
                    <CardContent className="p-8 flex flex-col flex-1">
                      <div
                        className={`w-14 h-14 rounded-2xl bg-gradient-to-br ${
                          colors[i % colors.length]
                        } flex items-center justify-center text-white mb-6 group-hover:scale-110 transition-transform shadow-lg`}
                      >
                        <Crown className="w-7 h-7" />
                      </div>

                      <h3 className="text-2xl font-bold mb-2">
                        {t(m.name_ar || m.name, m.name)}
                      </h3>
                      {m.description && (
                        <p className="text-muted-foreground text-sm mb-6">
                          {t(m.description_ar || m.description, m.description)}
                        </p>
                      )}
        ...
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              {selectedMembership &&
                t(
                  `الاشتراك في ${
                    selectedMembership.name_ar || selectedMembership.name
                  }`,
              ...
              <div className="p-4 rounded-xl bg-muted/50 border border-border">
                <div className="flex justify-between items-center mb-2">
                  ...
                  <span className="font-bold text-primary">
                    {selectedMembership.price} {t("ريال", "SAR")}
                  </span>
                </div>
          ...
          <DialogFooter>
            ...
            <Button
              ...
              disabled={isSubscribing || !paymentMethod}
              className="gap-2"
            >
              {isSubscribing ? (
